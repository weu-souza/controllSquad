...
export class SquadComponent implements OnInit
{
    ...
    public async cadastrar()
    {
        ...
        const [_, error] = await tryRunPromise(this.service.createSquad(this.squadForm.value));

        if (error)
        {
            this.snackbar.open('Não foi possível cadastrar a SQUAD', undefined, { duration: 5000 });
            return;
        }

        this.snackbar.open('SQUAD cadastrada com sucesso', undefined, { duration: 5000 });
